...
def get_image(driver,div):
    ...
    for background_image in background_images:
        ...
        print "@@@@@@@@@@@@@@@@@@"
        # 同事修改
        # location['x'] = int(re.findall('background-position:\s(.*?)px\s(.*?)px;', background_image.get_attribute('style'))[0][0])
        # location['y']=int(re.findall("background-position:\s(.*?)px\s(.*?)px;",background_image.get_attribute('style'))[0][1])       
...
def is_similar(image1,image2,x,y):
    ...
    for i in range(0,3):
        print pixel1[i]-pixel2[i], "##########", i
        # 如果相差超过94则就认为找到了缺口的位置
        # 或者50，自己修改
        if abs(pixel1[i]-pixel2[i])>=94:
            return False
    return True
