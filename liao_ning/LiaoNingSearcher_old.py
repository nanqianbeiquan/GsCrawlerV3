...
class LiaoNingSearcher(Searcher):
    ...
    def __init__(self):
        super(LiaoNingSearcher, self).__init__(use_proxy=False)
    ...
    def set_config(self):
        self.plugin_path = os.path.join(sys.path[0], '../liao_ning/ocr/liaoning.bat')
    ...
    def get_tag_a_from_page(self, keyword, flags=True):
        for t in range(10):
            yzm = self.get_yzm()
            url = 'http://gsxt.lngs.gov.cn/saicpub/entPublicitySC/entPublicityDC/lngsSearchFpc.action'
            params = {'authCode': yzm.encode('utf-8', 'ignore'), 'solrCondition': keyword}
        ...
        if len(search_result_list) > 0:
            ...
            # print 'search_result_json', search_result_json
    ...
    def get_search_args(self, tag_a, keyword):
        ...
        self.pripid = search_result_json['pripid']
        self.enttype = search_result_json['enttype']
    ...
    def get_ji_ben(self):
        ...
        url = 'http://gsxt.lngs.gov.cn/saicpub/entPublicitySC/entPublicityDC/getJbxxAction.action'
        params = {'pripid': self.pripid, 'type': self.enttype}
        r = self.get_request(url=url, params=params)
        soup = BeautifulSoup(r.text, 'lxml')
        td_list = soup.select('table.detailsList tr > td')
        th_list = soup.select('table.detailsList tr > th')[1:]
        result_json = [{}]
    ...
    def get_gu_dong(self):
        ...
        table_id = '04'
        self.json_result[family] = []
        url = 'http://gsxt.lngs.gov.cn/saicpub/entPublicitySC/entPublicityDC/getTzrxxAction.action'
        params = {'pripid': self.pripid, 'type': self.enttype}
    ...
    def get_bian_geng(self):
        ...
        table_id = '05'
        self.json_result[family] = []
        url = 'http://gsxt.lngs.gov.cn/saicpub/entPublicitySC/entPublicityDC/getBgxxAction.action'
        params = {'pripid': self.pripid, 'type': self.enttype}
        result_json = self.get_result_json(url, params)
        for j in result_json:
            self.json_result[family].append({})
    ...
    def get_zhu_yao_ren_yuan(self):
        ...
        table_id = '06'
        self.json_result[family] = []
        url = 'http://gsxt.lngs.gov.cn/saicpub/entPublicitySC/entPublicityDC/getZyryxxAction.action'
        params = {'pripid': self.pripid, 'type': self.enttype}
        result_json = self.get_result_json(url, params)
        for j in result_json:
            self.json_result[family].append({})
